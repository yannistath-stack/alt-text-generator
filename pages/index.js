...
export default function AltTextGenerator() {
  ...
  // ---- DataURL helper (guarantees data:image/... for the API) ----
  async function getImageDataUrl(item) {
    ...
    // Only have a blob: URL
    if (!blob && typeof item?.url === "string" && item.url.startsWith("blob:")) {
      blob = await fetch(item.url).then((r) => r.blob());
    }
    if (!blob && typeof item?.src === "string" && item.src.startsWith("blob:")) {
      blob = await fetch(item.src).then((r) => r.blob());
    }

    // Or a File
    if (!blob && item?.file instanceof Blob) {
      blob = item.file;
    }
  ...
  const processZipFile = async (file) => {
    ...
    try {
      ...
      for (const item of uniques) {
        setImages((prev) =>
          prev.map((p) =>
            p.id === item.id ? { ...p, processing: true } : p
          )
        );

        // 1) Build guaranteed data URL
        let dataUrl;
        try {
          dataUrl = await getImageDataUrl(item);
          // console.log("DATAURL_PREFIX:", String(dataUrl).slice(0, 30));
        } catch (e) {
      ...
      <div style={styles.card}>
        <div style={styles.headerSection}>
          ...
          <p style={styles.description}>
            Generate optimized alt text for automotive images
          </p>
        </div>
          ...
          <div style={styles.grid}>
            ...
            <div style={styles.inputGroup}>
              ...
              <input
                ...
                onChange={(e) =>
                  setVehicleInfo({ ...vehicleInfo, trim: e.target.value })
                }
                placeholder="Type S"
                style={styles.input}
              />
            ...
            >
              ...
              <p style={styles.uploadText}>
                {processing
                  ...
                  : dragActive
                  ? "Drop ZIP file here"
                  : "Drag and drop ZIP file or click to browse"}
              </p>
              <p style={styles.uploadSubtext}>
                {requiredFilled
                  ? "Supports JPG, PNG, WEBP, AVIF"
                  : "Please fill in Year and Model first"}
              </p>
            </label>
          </div>
        </div>
